...
async def _compress_messages(
    ...
    Sprint 17: uses chat_with_fallback (summariser → reporter → inference).
    Replaced the direct HTTP call with the standard llm_provider path so that
    the fallback cascade and circuit breaker apply here too.
    """
    ...
    logger.info(
        f"Compression request via fallback chain: "
        f"{[c['_slot_name'] for c in chain]}, msg_count={len(llm_messages)}"
    )

    try:
        response = await llm.chat_with_fallback(llm_messages, chain)
        ...
        logger.error(f"Compression failed (all slots exhausted): {e}")
        return ""
