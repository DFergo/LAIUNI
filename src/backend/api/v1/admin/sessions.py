...
async def generate_document(token: str, doc_type: str, _: dict = Depends(require_admin)):
    ...
    try:
        if doc_type == "summary":
            # User-facing summary — respects use_reporter_for_user_summary toggle
            summary_slot = "reporter" if settings.get("use_reporter_for_user_summary") else "inference"
            prompt_file = f"session_summary_{role}.md"
            content = await _generate_document(token, prompt_file, language, settings, slot=summary_slot)
            if not content:
                # Fallback to generic
                content = await _generate_document(token, "session_summary.md", language, settings, slot=summary_slot)
        elif doc_type == "internal_summary":
            content = await _generate_document(token, "session_summary_uni.md", language, settings, slot="reporter")
        elif doc_type == "report":
            content = await _generate_document(token, "internal_case_file.md", language, settings, slot="reporter")
        else:
            content = None
