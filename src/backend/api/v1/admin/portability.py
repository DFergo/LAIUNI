...
async def _post_frontend_import(frontend_id: str):
    """Rebuild RAG from imported source docs and push config to the sidecar."""
    try:
        from src.services.rag_service import reindex_campaign
        reindex_campaign(frontend_id)
    except Exception as e:
        logger.warning(f"Post-import reindex failed for {frontend_id}: {e}")
    try:
...
async def import_global(file: UploadFile = File(...), _: dict = Depends(require_admin)):
    ...
    try:
        from src.services.rag_service import reindex
        reindex()
    except Exception as e:
        logger.warning(f"Global reindex after import failed: {e}")
