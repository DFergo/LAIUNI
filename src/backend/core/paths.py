...
stay at the root too.
    ...
    ├── sessions/{token}/
    ├── campaigns/{fid}/   # per-frontend: config, branding, prompts, rag, llm overrides
    ├── deleted/{fid}/     # soft-deleted, recoverable
    ├── .admin_hash        # secret (root)
    ├── .jwt_secret        # secret (root)
    └── session_lifecycle.json  # runtime state (root)
"""
...
SESSION_LIFECYCLE = DATA_DIR / "session_lifecycle.json"
...
_MIGRATIONS: list[tuple[Path, Path]] = [
    ...
    (DATA_DIR / "documents", DOCUMENTS_DIR),
]
